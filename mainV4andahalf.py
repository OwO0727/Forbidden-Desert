...
def main(window):
    ...
    for row in range(GAME_BOARD_SIZE):
        for col in range(GAME_BOARD_SIZE):
            ...
            if any(tile['location'] == [row, col] for tile in special_tiles):
                ...
                game_board[row][col] = {"back": getImage(tile_info["back"]), "front": getImage(tile_info["front"]),
                                        "sand_markers": 0}
            ...
            # the rest of the tiles
                ...
                game_board[row][col] = {"back": getImage(tiles[0]), "front": getImage(tiles[normalGearIndex]),
                                        "sand_markers": 0}
    ...
    def locating_parts():
        ...
        for i in range(0, 8, 2):
            ...
            if game_board[location1_row][location1_col]["excavate"] == True and \
                    game_board[location2_row][location2_col]["excavate"] == True:
    ...
    for row in range(GAME_BOARD_SIZE):
        for col in range(GAME_BOARD_SIZE):
            ...
            button = tk.Button(window, text="", image=backOfCard, width=120, height=120, compound="center",
                               command=lambda rw=row, cl=col: selectTile([rw, cl]))  # ! CHANGED
    ...
    def update_func_display(funcs: dict):
        ...
        buttonsDF = [tk.Button(window, text=v, command=playerDefaultFuncs[v], width=8, height=8) for v in
                     playerDefaultFuncs.keys()]
        # for _b in buttonsDF: print()
        for i, v in enumerate(buttonsSF): v.grid(row=i % GAME_BOARD_SIZE,
                                                 column=6 + len(playerDefaultFuncs) // 5 + (i // GAME_BOARD_SIZE))
        for i, v in enumerate(buttonsDF): v.grid(row=i % GAME_BOARD_SIZE,
                                                 column=7 + len(playerDefaultFuncs) // 5 + (i // GAME_BOARD_SIZE))

    def do_turn():
        nonlocal turnNum, players, turnActions, current_player

        which = turnNum % len(players)
        current_player = players[which]
        c.create_text(710, 500, text=f"Player {which+1}", fill="black", font=('Helvetica 20 bold underline'))
        print(f"Player {which+1}")
        newActions = {m: getattr(current_player, m) for m in dir(current_player) if
                      not m.startswith("_") and callable(getattr(current_player, m))}
        turnActions = 4
