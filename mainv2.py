...
def main(window):
    ...
    #! WIP
    """
    def processTileImage(tile):
        if tile["excavate"]:
            baseImg = Image.open(tile["front"])
    ...
    PartD = 0
    def locating_parts():
        nonlocal PartA, PartB, PartC, PartD
    ...
    for row in range(GAME_BOARD_SIZE): 
        for col in range(GAME_BOARD_SIZE): 
            ...
            button = tk.Button(window, text="", image = backOfCard, width=120, height=120, compound="center", command = lambda rw=row, cl=col: movePlayer(rw,cl)) # ! CHANGED
    ...
    def movePlayer(row, col):
        nonlocal turnNum, players, turnActions
        currPlayer = players[turnNum%len(players)]
        if True in [abs(currPlayer.position[0] - row) + abs(currPlayer.position[1] - col) >= 2, 
                    game_board[row][col]["sand_markers"] >= 2,
                    game_board[currPlayer.position[0]][currPlayer.position[1]]["sand_markers"] >=2
                    ]:
            print("cant move")
            return
        currPlayer.position=[row,col]
        turnActions-=1
        if turnActions==0: end_turn()
    ...
    def update_func_display(funcs:dict):
        buttonsF = [tk.Button(window, text=v, command=lambda: [print("action"), end_turn(), update_board_display()], width=8, height=8) for v in funcs.keys()]
        for i,v in enumerate(buttonsF): v.grid(row=i%GAME_BOARD_SIZE, column=5+(i//GAME_BOARD_SIZE))
    
    def do_turn():
        nonlocal turnNum, players, turnActions
        
        thisPlayer = players[turnNum%len(players)]
        newActions = {m:getattr(thisPlayer, m) for m in dir(thisPlayer) if not m.startswith("__") and callable(getattr(thisPlayer, m))}
        turnActions=4
